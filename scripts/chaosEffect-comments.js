...
function setTheme() {
  var exemplarTheme = "";
  if (document.getElementById("allThemes").checked === true) {
    // if it's "all", set it to "all"
    exemplarTheme = "all";
  } else {
...
function setRM() {
  var customRarityMatters = document.getElementById(
    "customRarityMattersToggle"
  ).checked;
...
function makeGameList() {
  return effectLibrary.filter((obj) => {
    return (
      ...
      (!filterCriteria.accessible ||
        obj.accessible.includes(filterCriteria.accessible)) && // if accessible is defined and matches
      (!filterCriteria.inclusion ||
        obj.inclusion.includes(filterCriteria.inclusion)) && // if inclusion is defined and matches
      (!filterCriteria.exemplarTheme ||
        obj.exemplarTheme.includes(filterCriteria.exemplarTheme)) && // if theme is defined and matches
      (!filterCriteria.school || obj.school.includes(filterCriteria.school)) && // if school is defined and matches
      (!filterCriteria.duration ||
        obj.duration.includes(filterCriteria.duration)) && // duration
